...
int tfs_fill_sb (struct super_block * sb, void * data, int silent) {
	...
	struct dentry *root_dentry, *son_dentry;
    ...
    printk(KERN_ALERT "number of cpus is %i\n", getNumCPU());

    cpulimit = getNumCPU();
    for (i = 0; i < cpulimit; ++i) {
		name = kmalloc(15 * sizeof(char), GFP_KERNEL);
		sprintf(name, "CPU_%u", i);

		son_dentry = add_son(root_dentry, name);
    	build_dfs(CPUID_TO_LISTID(i), CPUID_TO_LISTID(i), son_dentry, i, proc_list);
    }
...
int tfs_iterate (struct file * file, struct dir_context * ctx) {
	...
	// printk(KERN_ALERT "Iterate from file with path: %s\n", file->f_path.dentry->d_name.name);
	// printk(KERN_ALERT "The result of dir_emit dots is %i\n", (int)ret);
...
struct dentry * tfs_lookup (struct inode * parent_inode, struct dentry * here_dentry, unsigned int flags) {
	...
	list_for_each(curr_list_item, &(parent_dentry->d_subdirs)) {
		...
		if (strcmp(curr_dentry->d_name.name, here_dentry->d_name.name) == 0) {
			// *here_dentry = *curr_dentry; // doesn't seem to work
...
struct dentry * add_son(struct dentry * parent, const char * const p_name) {
